...
#define CONST_DEC_TYPE_ERROR 14                           //常量声明中，常量未赋值
...
FILE* output_file =fopen("D://1.txt", "w");                                         //词法分析结果输出的文件
void error(int error_type,int line_index){
    switch(error_type){
        ...
        case 14:{
            fprintf(output_file,"line %d: no value in cosntant defination!\n",line_index);
            printf("line %d: no value in cosntant defination!\n",line_index);
            break;
        }
        case 15:{
