...
char get_ch(){//读入字符 
	if(read_index == read_length){//该缓冲数组中的字符已经读完 
        if(fgets(read,256,input_file) != NULL){
...
int lexanal(){//单词的分析程序，进入之前CHAR已经读入了字符 
	...
	if(is_letter()) {//如果单词的首字符为字母 
		...
		type = DIGITSY;
		trans_num = trans_to_bina();//将数字转化为二进制数值 
		
		if((iszero == 1) && (trans_num != 0)){//首位为0的非零数字 
			printf("%-20s%-20s\n", token, "单词错误");
		}
		else 
			printf("%-20s%-20d%-20lld\n", token, type, trans_num);
	}
...
int main(){
	input_file = fopen("C:\\Users\\柳啸峰\\Desktop\\词法分析程序\\input.txt", "r");
